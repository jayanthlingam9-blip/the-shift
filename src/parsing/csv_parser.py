...
def parse_hcahps_csv(
    ...
    chunks_dir = PROJECT_ROOT / "data" / "chunks" / domain / "csv"
    ...
    # CSV retrieval units also live under data/chunks/, alongside the
    # hierarchical pdf/html chunks, so data/chunks/ is the single home for all
    # retrieval units. Same JSONL records, written in the one streaming pass.
    chunk_profiles_path = chunks_dir / f"{csv_path.stem}.hospital_profiles.jsonl"
    chunk_category_docs_path = (
        chunks_dir / f"{csv_path.stem}.hospital_category_docs.jsonl"
    )
    ...
    with (
        ...
        chunk_profiles_path.open("w", encoding="utf-8") as chunk_profiles_file,
        chunk_category_docs_path.open("w", encoding="utf-8") as chunk_category_docs_file,
    ):
        def emit_outputs(profile: dict, category_docs: list[dict]) -> None:
            write_json_line(profiles_file, profile)
            write_json_line(chunk_profiles_file, profile)
            for category_doc in category_docs:
                write_json_line(category_docs_file, category_doc)
                write_json_line(chunk_category_docs_file, category_doc)
        ...
        for source_row_number, row in enumerate(reader, start=2):
            ...
            if facility_id != current_facility_id:
                ...
                emit_outputs(profile, category_docs)
